...
function renderLinkedCard(row) {
  ...
  return `<article class="votes-member-card">
    ...
    <div class="votes-member-card__footer"><span>${aliases.length} pseudo${aliases.length > 1 ? "s" : ""}</span><span>${(row.matchedNames || []).length} retrouvé${(row.matchedNames || []).length > 1 ? "s" : ""}</span><button type="button" class="votes-history-button" data-history-steamid="${escapeHtml(row.steamId)}" data-history-name="${escapeHtml(row.playerName)}" title="Voir toutes les opérations de cagnotte liées à ce SteamID">HISTORIQUE CAGNOTTE</button></div>
  </article>`;
}
