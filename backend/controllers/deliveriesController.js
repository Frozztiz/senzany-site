...
function normalizeItems(items) {
  ...
  return items
    .map((item) => {
      ...
      const maxQuantity = className === "SenzanyBankCredit" ? 100000000 : 999;

      return {
        className,
        name: className === "SenzanyBankCredit" ? "Crédit bancaire" : className,
        quantity: Number.isFinite(quantity)
          ? Math.max(1, Math.min(maxQuantity, Math.floor(quantity)))
          : 1,
        metadata:
